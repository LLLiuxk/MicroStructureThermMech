...
//    Eigen::MatrixXd X = solver.solve(F.block(2, 0, ndof - 2, 3)); // 求解线性方程组
...
//    }
//
//    // 提取 F 的列数据：列0取 sub_rows 行，列1取 sub_rows_F2 行 (MATLAB: 4:2:end → 3,5,7...)
...
//        F_sub(i, 0) = F.coeff(sub_rows[i], 0);  // 第一列
//        F_sub(i, 1) = F.coeff(sub_rows_F2[i], 1); // 第二列
//    }
//
//
//    std::vector<int> K_rows_cols; // k目标行列索引 
//   
...
//        int global_row = sub_rows[i]; // 对应原始行号
